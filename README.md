# EllaBox – AI Health Companion & Cloud Dashboard
...
The companion **web dashboard** (a single HTML file) lets you monitor sensor data, view health trends, set reminders, and configure the device remotely.

![EllaBox UI Preview](docs/ui_preview.jpg)  
...
- Real‑time sensor display (heart rate, SpO₂, temperatures, humidity, AQI, TVOC, eCO₂)
- Interactive charts (last 50 readings) with gradient fills and time‑based x‑axis
- **Smart Reminders** – Add, edit, and delete reminders (medication, exercise, general). They sync with EllaBox and are spoken at the correct time.
- **User Profile** – Store name, age, emergency contact, Telegram bot token/chat ID. Syncs to Firebase and is used by the device for alerts and personalisation.
- **Remote Wi‑Fi Configuration** – Send new SSID/password to EllaBox (device reboots to connect).
- **Emergency Button** – Triggers a distress alert on the device and sends a Telegram message.
...
- `EllaBox.ino` – main Arduino sketch
- `index.html` – the web dashboard (single file, ready to host)

### 3. Configure Credentials (Arduino Sketch)

Edit the top of `EllaBox.ino` to set your Wi‑Fi and API keys:
...
- The bot token and chat ID can be entered later via the web dashboard (they will be synced to Firebase).

### 4. Firebase Data Structure

The sketch and dashboard expect the following paths in your Realtime Database:
...
/reminders
  - id1: { detail: "Take medicine", time: "14:30", type: "medication" }
  - id2: { detail: "Water plants", time: "09:00", type: "general" }
...
The dashboard is a single HTML file (`index.html`) that you can host on any static web server (GitHub Pages, Netlify, or even locally). It communicates directly with your Firebase Realtime Database.

### Hosting on GitHub Pages

1. Push `index.html` to a GitHub repository.
2. Go to **Settings → Pages** and enable GitHub Pages from the `main` branch.
3. Your dashboard will be available at `https://yourusername.github.io/repo-name/`.

### Local Usage

Just open `index.html` in a modern browser (Chrome, Edge, Firefox). It will connect to Firebase as long as you have an internet connection.

### Dashboard Features

- **Real‑time Sensor Cards** – Updated whenever new data arrives from EllaBox.
- **Health Charts** – Heart rate, SpO₂, body temperature, room temperature, humidity, AQI (last 50 readings).
- **Reminders Manager** – Add, edit, delete reminders. They are stored in Firebase and automatically spoken by EllaBox at the scheduled time.
- **User Profile** – Set your name, emergency contact, Telegram credentials. These are used by EllaBox for personalised alerts.
- **Remote Wi‑Fi Configuration** – Send new SSID/password to EllaBox (device will reboot and connect to the new network).
- **Emergency Button** – Triggers a distress alert on the device and sends a Telegram message.
...
Once the bot token and chat ID are set (via the web dashboard), you can send commands to your bot:
...
1. In the web dashboard, go to **History** → expand **Settings & Profile**.
2. Enter the new SSID and password in the **WiFi Update** section.
3. Click **Save & Sync**. EllaBox will read the new credentials, save them to Preferences, and restart automatically.

Similarly, the user profile (name, age, emergency contact, Telegram credentials) can be updated from the dashboard and will be used by EllaBox for alerts and personalisation.
...
- **Dashboard not updating** – Ensure Firebase rules allow read/write. For development, you can set:
